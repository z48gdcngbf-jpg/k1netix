...
if _running_in_streamlit():
    # Use Layer 1 output directly. Do not open bcf_output.json here; Streamlit
    # already has the parsed BCF data in memory.
    render_layer2_ui(st.session_state.get("bcf_json", data))
elif __name__ == "__main__":
    import sys
    cli_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).with_name("bcf_output.json")
    with open(cli_path, encoding="utf-8") as f:
        cli_data = json.load(f)

    result = run_layer2(cli_data)
    out_path = cli_path.with_name("k1netix_layer2.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(json.loads(_export_json(result)), f, indent=2, default=str)
